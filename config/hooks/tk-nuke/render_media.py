...
class RenderMedia(HookBaseClass):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._burninWindows_nk = os.path.join(
            self.__app.disk_location, "resources", "burninWindows.nk"
        )
    ...
    def render(
        ...
        try:
            ...
            vectorfield.knob('colorspaceOut').setValue(5)
            vectorfield.setInput(0, read)

            # now create the slate/burnin node
            if platform.system() == 'Windows':
                burn = nuke.nodePaste(self._burninWindows_nk)
                burn.setInput(0, vectorfield)
                ...
                slate_str += "Vendor: GLASSWORKS\n"
                slate_str += "Media color: Rec 709\n"
                # slate_str += "VFX Scope of work: 2D Compositing\n"

                shot_data = sg.find_one("Shot", [["id", "is", ctx.entity["id"]]], ["sg_vfxscopeofwork"])
                # adding data from sg submission note column
                data = {
                    "sg_vfxscopeofwork": (shot_data["sg_vfxscopeofwork"] or "")
                }
                sgScpWork = data["sg_vfxscopeofwork"]
                slate_str += "VFX Scope of work: " + sgScpWork + "\n"
                ...
                if nuke.ask("is this a WIP? If you select no it will be send as FINAL"):
                    slate_str += "Submit for: WIP\n"
                else:
                    slate_str += "Submit for: FINAL\n"

                # shot_data = sg.find_one("Shot", [["id", "is", ctx.entity["id"]]], ["sg_submissionnote"])
                # adding data from sg submission note column
                # data = {
                #    "sg_submissionnote": (shot_data["sg_submissionnote"] or "")
                # }
                # sgSubNote = data["sg_submissionnote"]
                # slate_str += "Submission note: " + sgSubNote + "\n"


                slate_str += "Frames: %s - %s\n" % (first_frame, last_frame)

                burn.node("slate_info")["message"].setValue(slate_str)
                burn.node("slate_info")["leading"].setValue(0.6)
                ...
                # timecode.knob('startcode').setValue('00:00:41:16')
                # timecode.setInput(0, burn)
        ...
        if output_node:
            ...
            nuke.executeMultiple(
                [output_node], ([1024, last_frame - 24, 1],), [nuke.views()[0]]
            )
    ...
    def __create_output_node(self, path):
        ...
        # node.knob(file).setValue(quicktimeRender)
