...
+
    ...
    def publish(self, settings, item):
        ...
        # add comment to sg_team_notes_tab
        currentEngine = sgtk.platform.current_engine()
        ctx = currentEngine.context
	entity = ctx.entity
        description = publish_data["comment"]
        #shot_data = sg.find_one("Shot", [["id", "is", ctx.entity["id"]]], ["sg_team_notes"])
        # adding data from sg submission note column
        data = {
            "sg_team_notes": (description or "")
        }
        sg.update("Shot", ctx.entity['id'], data)

        #update the shot status when publishing to pending review
        dataStatus = {
            "sg_status_list": ('rev')
        }
        sg.update("Shot", ctx.entity['id'], dataStatus)

	#update the data status when publishing to pending review
        filters = [['entity', 'is', {'type':'Shot', 'id':entity['id']}]]
        fields = ['content']
        result = sg.find("Task", filters, fields)
        resultDict = (result[0])
        taskId = resultDict['id']
        dataStatus = {"sg_status_list": ('rev')}
        sg.update("Task", taskId, dataStatus)
